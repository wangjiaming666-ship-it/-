...
专科智能体是本系统中最关键的推理单元。它不应只在路由之后直接给出药物推荐，而应先读取对应专科知识库，完成本专科视角下的诊断评估，再形成分层用药建议。
...
- 推荐药物 TopK。
- 分层用药建议，包括疾病直接治疗、风险控制、支持/对症治疗、住院通用药物和需复核药物。
- 推荐理由。
- 风险提醒。
- 低优先级或应避免药物。
- 置信度。
...
第三轮是共识生成。系统根据各专科保留意见、交叉审阅反馈、初始主专科和候选建议支持情况，生成 MDT 共识方案、主专科优先修订方案和保守低冲突方案。每个方案保留兼容性的 `drugs` 药物列表，同时新增 `medication_layers` 字段，将药物分为疾病直接治疗、风险控制、支持/对症治疗、住院通用药物和需复核药物。
...
- 药名一致性：比较系统最终推荐药物与 MIMIC-IV 真实医生处方的重合情况，报告 Precision、Recall、F1 和 Jaccard。
- 治疗角色覆盖率：将系统推荐药物和真实医生处方映射到治疗角色，评价系统是否覆盖真实处方中的主要治疗方向。

由于 MIMIC-IV 真实处方包含护理用药、支持治疗和临时医嘱，本文不将药名完全一致作为唯一标准，而将治疗角色覆盖率作为更重要的合理性指标。流程完整性、风险提示和 MDT 交叉审阅结果作为案例分析的辅助说明。
